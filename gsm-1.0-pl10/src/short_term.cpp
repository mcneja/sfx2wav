...
(
	...
	s16 * LARp)
{
	for (int i = 0; i < 8; ++i, LARp++, LARpp_j_1++, LARpp_j++)
	{
		*LARp = GSM_ADD(*LARpp_j_1 / 4, *LARpp_j / 4);
		*LARp = GSM_ADD(*LARp, *LARpp_j_1 / 2);
	}
}
...
(
	...
	s16 * LARp)
{
	for (int i = 0; i < 8; ++i, LARpp_j_1++, LARpp_j++, LARp++)
	{
...
{
	for (int i = 0; i < 8; ++i, LARp++, LARpp_j++)
		*LARp = *LARpp_j;
}

/* 4.2.9.2 */

static void LARp_to_rp (s16 * LARp)	/* [0..7] IN/OUT  */
/*
 ...
 */
{
	for (int i = 1; i <= 8; i++, LARp++)
	{
		...
		if (*LARp < 0)
		{
			s16 temp = *LARp == MIN_WORD ? MAX_WORD : -(*LARp);
...
(
	gsm_state * S,
	const s16 * rrp, // [0..7]   IN
	int	        k,   // k_end - k_start
	const s16 * wt,	 // [0..k-1] IN
	s16 *       sr   // [0..k-1] OUT
)
{
	s16 * v = S->v;
	...
	for (; k > 0; --k)
	{
		...
		for (int i = 8; i--;)
		{
			...
			if (tmp1 == MIN_WORD && tmp2 == MIN_WORD)
				...
				tmp2 = s16(((s32(tmp1) * s32(tmp2) + 16384) / 32768) & 0xFFFF);
			...
			if (tmp1 == MIN_WORD && sri == MIN_WORD)
				...
				tmp1 = s16(((s32(tmp1) * s32(sri) + 16384) / 32768) & 0xFFFF);
...
(
	...
	const s16 * LARc, // coded log area ratio [0..7]  IN
	s16 *       s     // signal [0..159]		IN/OUT
)
{
	s16 * LARpp_j	= S->LARpp[ S->j      ];
	s16 * LARpp_j_1	= S->LARpp[ S->j ^= 1 ];
...
(
	...
	const s16 * LARcr, // received log area ratios [0..7] IN
	const s16 * wt,    // received d [0..159]             IN
